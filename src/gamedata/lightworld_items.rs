...
pub fn try_get_lightworld_item_name(item_id: u32) -> Option<&'static str> {
    match item_id {
        0 => Some("---"),
...
pub fn display_lightworld_item(item_id: u32) -> CompactString {
    let item_name = try_get_lightworld_item_name(item_id);
    item_name.map(|n| n.to_compact_string()).unwrap_or_else(|| format_compact!("LW Item {}", 
        item_id))
}
