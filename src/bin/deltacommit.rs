...
fn main() {
    ...
    if !custom_message.is_empty() {
        ...
        for i in 0..custom_message.len() {
            custom_message[i] = custom_message[i].clone() + "\n";
        }
    }
    ...
    let commit_result = Command::new("git").arg("commit").arg("-m").arg(&commit_message).output();
    ...
    info!("Committed successfully. Room {}, played for {}.", room_name, time_played);
}
